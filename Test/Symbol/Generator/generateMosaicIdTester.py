...
"""
    for i, c in enumerate(j):
        ...
        body += indent(f'var mainAd = MainFacade.TransactionFactory.Network.PublicKeyToAddress(Converter.HexToBytes(publicKey));\n')
        body += indent(f'var mainAddress = Converter.AddressToString(mainAd.bytes);\n')
        body += indent(f'var testAd = TestFacade.TransactionFactory.Network.PublicKeyToAddress(Converter.HexToBytes(publicKey));\n')
